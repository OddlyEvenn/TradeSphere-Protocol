...
import { BadgePercent, TrendingUp, ShieldCheck, AlertCircle, FileText, Search } from 'lucide-react';
        ...
        <div className="space-y-10">
            ...
            <div className="card-premium !p-0 overflow-hidden">
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50">
                            <tr>
                                ...
                                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Value (USD)</th>
                                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Duty Est.</th>
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {trades.map(trade => (
                                <tr key={trade.id}>
                                    ...
                                    <td className="px-8 py-6 font-bold text-indigo-600">${(trade.amount * 0.05).toLocaleString()}</td>
                                    <td className="px-8 py-6 text-right">
                                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${trade.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                                            {trade.status}
                                        </span>
                                    </td>
                                </tr>
                ...
                {trades.length === 0 && !loading && (
                    <div className="p-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-900">No Monitored Activity</h3>
            ...
            <div className="bg-slate-900 rounded-[2.5rem] p-12 text-white overflow-hidden relative shadow-2xl shadow-indigo-200/50">
                <div className="relative z-10">
                    <h2 className="text-2xl font-black mb-2 uppercase tracking-tighter">Automated Tax Settlement</h2>
                    <p className="text-slate-400 max-w-md mb-8">All customs duties and VAT are automatically calculated and held in smart contract escrows during transit.</p>
                    <button className="btn-primary bg-white text-slate-900 px-8 hover:bg-slate-100 transition-all">Audit Global Log</button>
                </div>
