...
const getStatusColor = (status: string) => {
    ...
    if (['DOCS_VERIFIED', 'CUSTOMS_CLEARED', 'LOC_VERIFIED'].includes(status)) return 'bg-blue-50 text-blue-700';
    if (['GOODS_SHIPPED', 'DOCS_SUBMITTED', 'SHIPPING_NOMINATED'].includes(status)) return 'bg-indigo-50 text-indigo-700';
                            ...
                            {trades.map(trade => (
                                <tr key={trade.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-8 py-5">
                                        <p className="font-black text-slate-900 text-sm">#{trade.blockchainId || trade.id.slice(0, 8)}</p>
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{trade.productName || '—'}</p>
                                    </td>
                                    <td className="px-8 py-5">
